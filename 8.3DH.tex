...
\begin{exampleblock}{Are the following statements true or false?}
\begin{itemize}
\item $\mathbb{Z}_{15}$ with `$+$' is cyclic and the `$1$' is a generator.
\item For $\mathbb{Z}_7^*$, $2$ is a generator, but $3$ is not.
\end{itemize}
\end{exampleblock}
\begin{exampleblock}{}
...
\]
However, $\mathbb{Z}^*_p$ does not have prime order.
\end{frame}
...
$x = 3, y = 4$, what's the message from Bob to Alice?\\ %3^{4} = 4
