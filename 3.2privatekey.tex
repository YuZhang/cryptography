...
\end{definition}
\begin{theorem}
$\Pi = (\mathsf{Gen}, \mathsf{Enc}, \mathsf{Dec})$ for which $\mathsf{Enc}$ is a deterministic function of the key and the message. Then $\Pi$ does not have indistinguishable multiple encryptions in the presence of an eavesdropper.
\end{theorem}
...
\end{center}
\end{figure}
Unsynchronized mode: $\mathsf{Enc}(m) := \left< IV, G(k, IV) \oplus m\right>$.\\
Initial vector $IV$ is chosen \emph{u.a.r}.
\end{frame}
\begin{frame}\frametitle{Related Keys: Real World Cases}
Keys for multiple enc. must be independent.
\begin{exampleblock}{802.11b WEP}
...
\item \textbf{Look-up table $f$}: $\{0,1\}^n \to \{0,1\}^n$ with size $n\cdot2^n$.
...
\textbf{Intuition}: A PRF $F$ generates a function $F_k$ that is indistinguishable for a \textsc{ppt} distinguisher $D$ from truly random selected function $f$ (look-up table) in $\mathsf{Func}_n$.\\ However, the function has \textbf{exponential length}. Give $D$ the deterministic \textbf{oracle access $D^{\mathcal{O}}$} to the functions $\mathcal{O}$.
\begin{definition}
...
\end{definition}
\end{frame}
\begin{frame}\frametitle{Block Cipher and Modes of Operation}
\begin{itemize}
\item \textbf{Block cipher}: A practical (strong) PRP.
\begin{itemize}
\item Not encryption scheme, but a building block of encryption scheme.
\item Block cipher itself is not CPA-secure.
\end{itemize}
\item \textbf{Modes of Operation}: A way of encrypting arbitrary-length messages using a block cipher/PRP.
\end{itemize}
\end{frame}
...
\end{figure}
\begin{itemize}
\item \textbf{Weakness}: not indistinguishable in the presence of an eavesdropper.
\end{itemize}
\end{frame}
...
\end{figure}
\begin{itemize}
\item \textbf{Strength}: CPA-secure.
\item \textbf{Weakness}: not parallelizable.
\end{itemize}
\end{frame}
...
\end{figure}
\begin{itemize}
\item \textbf{Strength}: CPA-secure, pre-processing the stream, PRF-compatible.
\item \textbf{Weakness}: not parallelizable.
\end{itemize}
\end{frame}
...
\end{figure}
\begin{itemize}
\item \textbf{Strength}: CPA-secure, pre-processing the stream, PRF-compatible, parallelizable, random access.
\end{itemize}
\end{frame}
...
Input \verb#ivec# should be random, otherwise it is not secure.
\end{exampleblock}
\end{frame}
...
\end{itemize}
\item None of the above scheme is CCA-secure: If one bit of CT is flipped, so does one bit of PT. 
