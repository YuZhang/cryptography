...
16. 定义哈希函数（**Hash Function**）
    ...
    - 一个哈希函数 (压缩函数) 是一对PPT算法 $(\mathsf{Gen}, H)$ 满足以下条件:
      -  a key $s \gets \mathsf{Gen}(1^n)$， $s$ 不保密.
...
30. 信息论上MAC安全定义
    ...
    - 不可能定义“完美的”MAC，由于敌手可以至少以$1/2^{|t|}$ 的概率输出一个有效的标签。
    - 一次MAC实验 $\mathsf{Macforge}^{\mathsf{1-time}}_{\mathcal{A},\Pi }$: 敌手查询一次MAC预言机后输出消息和标签，
      - $k \gets \mathsf{Gen}$.
...
31. 信息论上MAC的构造
    ...
    - SUF中不同输入会产生不同的输出，输出之间是完全独立的，因此也称为“pairwise-independent”；
    - 构造：
    ...
    - 由于SUF的特性，敌手查询一次后，伪造成功的概率只有$1/|\mathcal{T}|$。
...
33. 信息论MAC的局限性
    ...
    - 证明：直觉上，每对消息和标签成立需要$2^n$个密钥，才能保证 $2^{-n}$-安全。一共2对，需要$2^{2n}$。令 $\mathcal{K}(t) \overset{\mathsf{def}}{=} \{ k | \mathsf{Vrfy}_k(m, t) = 1\}$，即所有能产生指定消息和标签的密钥集合。对于任意 $t$， $|\mathcal{K}(t)| \leq 2^{-n} \cdot |\mathcal{K}|$。 否则， $(m, t)$ 是一个有效标签的概率至少为 $|\mathcal{K}(t)|/|\mathcal{K}|> 2^{-n}$，这于安全要求矛盾。 $\mathcal{A}$从第一次查询对应的密钥集合中选择一个密钥，并输出一个新消息的有效标签的概率是至少 $ \sum_{t} \Pr [\mathsf{Mac}_k(m) = t] \cdot \frac{1}{|\mathcal{K}(t)|} \geq \sum_{t} \Pr [\mathsf{Mac}_k(m) = t] \cdot \frac{2^n}{|\mathcal{K}|} = \frac{2^n}{|\mathcal{K}|} $ 。由于概率至多 $2^{-n}$, $|\mathcal{K}| \geq 2^{2n}$。由于所有密钥具有相同长度，每个密钥的长度至少是 $2n$。
