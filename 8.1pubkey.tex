...
\end{theorem}
\begin{proposition}
If $\Pi$ is secure in the presence of an eavesdropper, then $\Pi$ also is CPA-secure, and secure for multiple encryptions.
\end{proposition}
\begin{proposition}
...
\begin{itemize}
\item $\mathsf{Gen}$: $pk = I$, $sk = \mathsf{td}$.
\item $\mathsf{Enc}$: $r \gets D_I$ and compute $k := H(r)$, output $\langle f_I(r), \mathsf{Enc}'_k(m)\rangle$.
