...
Time $t$ and probability $\varepsilon$ are described as functions of \textbf{security parameter} $n$ (usually, the length of key).
\begin{itemize}
\item Time is \textsc{ppt} (\textbf{probabilistic polynomial-time}): $n^a$ for constant $a$.
\item Probability is \textbf{negligible}: smaller than any inverse polynomial $n^{-b}$ for constant $b$.
...
\item Random number generators should be designed for cryptographic use, not \verb|random()| in C.
...
\item $\mathsf{negl}_3(n) = \mathsf{negl}_1(n)+\mathsf{negl}_2(n)$ is negligible.
\item positive polynomial $p$, $\mathsf{negl}_4(n) = p(n)\cdot\mathsf{negl}_1(n)$ is negligible.
...
\end{definition}
\end{frame}
\begin{frame}\frametitle{Properties of Definition of Indistinguishable}
\begin{enumerate}
...
\item It is impossible to definitively prove randomness.
\end{itemize}
\begin{figure}
\begin{center}
...
\end{definition}
\end{frame}
\begin{frame}\frametitle{Remarks on Pseudorandom Generators}
\begin{itemize}
...
\end{proof}
\end{frame}
\begin{frame}\frametitle{Handling Variable-Length Messages}
